...
<div class="service-overview-card">
  ...
  <h3>Financial Services Overview</h3>
  <p>
    OLLSMPC provides reliable savings and loan services designed to support
    the financial stability, growth, and long-term goals of its members.
  </p>
...
<?php
$hero = mysqli_fetch_assoc(mysqli_query($conn, "
    SELECT * FROM services ORDER BY id DESC LIMIT 1
"));
?>
                <img src="<?= htmlspecialchars($hero['image']) ?>"  alt="Commercial Construction Services" class="img-fluid">
...
<div class="content-intro">
  <p>
    Our Lady of La Salette Multi-Purpose Cooperative (OLLSMPC) offers a comprehensive
    range of financial services designed to empower members, encourage savings
    discipline, and provide accessible credit solutions. Our services are built
    on the principles of trust, mutual assistance, and long-term financial growth.
  </p>
  <p>
    Through our savings and loan programs, members are given opportunities to
    strengthen their financial stability, support their families, and invest
    in personal or business development with confidence and security.
  </p>
</div>
...
<div class="methodology-section" data-aos="fade-up" data-aos-delay="300">
   <br><h2><?= htmlspecialchars($category['name']) ?></h2>

  <div class="methodology-timeline">

      <?php
      $services = mysqli_query($conn, 
          "SELECT * FROM services WHERE category_id=" . $category['id']
      );
      $counter = 1;
      ?>
    ...
    <div class="timeline-item">
      <div class="timeline-marker">
        <span class="phase-number"><?= $counter++ ?></span>
      </div>
      <div class="timeline-content">
        ...
        <p><?= nl2br(htmlspecialchars($service['description'])) ?></p>
            ...
            <ul class="phase-features">
              <?php
              $features = mysqli_query($conn,
                  "SELECT * FROM service_features WHERE service_id=" . $service['id']
              );
              while ($feature = mysqli_fetch_assoc($features)):
              ?>
                <li><?= htmlspecialchars($feature['feature_text']) ?></li>
              <?php endwhile; ?>
            </ul>
          </div>
        </div>
