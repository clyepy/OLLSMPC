...
// Escape id for SQL safely
$id_safe = mysqli_real_escape_string($conn, $id);

// Query the database
$res = mysqli_query($conn, "SELECT * FROM events WHERE id='$id_safe'");
  ...
  <main class="main">
    ...
    <div class="page-title light-background">
      <div class="container d-lg-flex justify-content-between align-items-center">
        <h1 class="mb-2 mb-lg-0">OLLSMPC Event Details</h1>
...
<?php if (isset($event['banner_image']) && trim($event['banner_image']) !== ''): ?>
  <img src="<?= htmlspecialchars($event['banner_image'], ENT_QUOTES) ?>"
       class="img-fluid"
       alt="Event Banner">
<?php endif; ?>
...
<h1 class="main-title"><?= htmlspecialchars($event['title'] ?? '') ?></h1>
<p class="summary-text"><?= htmlspecialchars($event['summary'] ?? '') ?></p>


<div class="key-metrics">
  <div class="metric-row">
    <div class="metric">
      <span class="metric-title">Event Date</span>
      <span class="metric-data"><?= htmlspecialchars($event['event_date'] ?? '') ?></span>
    </div>
  </div>
  <div class="metric-row">
    <div class="metric">
      <span class="metric-title">Event Type</span>
      <span class="metric-data"><?= htmlspecialchars($event['event_type'] ?? '') ?></span>
    </div>
    <div class="metric">
      <span class="metric-title">Organizer</span>
      <span class="metric-data"><?= htmlspecialchars($event['organizer'] ?? '') ?></span>
    </div>
  </div>
</div>
</div>
...
<div class="visual-showcase">
  <div class="showcase-grid">
    <?php foreach ($gallery as $index => $img): ?>
      <?php if (!empty($img)): ?>
        <div class="showcase-item <?= $index === 0 ? 'large' : '' ?>">
          <img src="<?= htmlspecialchars($img) ?>" class="img-fluid" loading="lazy" alt="Event image">
...
<div style="margin-top:20px;">
  <a href="event.php?type=<?= urlencode($event['event_type']) ?>" 
     class="btn btn-primary">
    See more <?= htmlspecialchars($event['event_type']) ?> events
  </a>
</div>
...
<div class="spec-table">
  <div class="spec-row">
    <span class="spec-name">Event Name</span>
    <span class="spec-detail"><?= htmlspecialchars($event['title'] ?? '') ?></span>
  </div>
  <div class="spec-row">
    <span class="spec-name">Organizer</span>
    <span class="spec-detail"><?= htmlspecialchars($event['organizer'] ?? '') ?></span>
  </div>

<div class="spec-row">
  <span class="spec-name">Purpose</span>
  <span class="spec-detail"><?= htmlspecialchars($event['purpose'] ?? '') ?></span>
</div>

<div class="spec-row">
  <span class="spec-name">Documentation</span>
  <span class="spec-detail"><?= htmlspecialchars($event['documentation'] ?? '') ?></span>
</div>
